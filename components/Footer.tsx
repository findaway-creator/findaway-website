import Link from 'next/link';
import Image from 'next/image';
import { colors } from '@/lib/colors';

export default function Footer() {
  return (
    <footer className="pt-20 pb-10" style={{ backgroundColor: colors.cream }}>
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:justify-between gap-12 mb-16">
          {/* Logo & tagline */}
          <div className="max-w-xs">
            <Link href="/" className="block mb-4">
              <Image src="/logos/logo.svg" alt="Find A Way" width={120} height={32} className="h-8 w-auto" />
            </Link>
            <p className="font-body text-lg leading-relaxed" style={{ color: `${colors.navy}90` }}>
              Helping you navigate the early years.
            </p>
          </div>

          {/* Link columns */}
          <div className="grid grid-cols-2 md:grid-cols-3 gap-8 md:gap-16">
            {/* Navigation links */}
            <div>
              <h4 className="font-body text-xs font-bold uppercase tracking-wider mb-4" style={{ color: colors.navy }}>
                About
              </h4>
              <ul className="space-y-3">
                {['About Us', 'Our Services', 'Our Team', 'Private Practice'].map((link) => (
                  <li key={link}>
                    <Link
                      href={`/#${link.toLowerCase().replace(/\s/g, '-')}`}
                      className="footer-link font-body text-sm transition-colors hover:underline"
                      style={{ color: `${colors.navy}80` }}
                    >
                      {link}
                    </Link>
                  </li>
                ))}
              </ul>
            </div>

            {/* Resources */}
            <div>
              <h4 className="font-body text-xs font-bold uppercase tracking-wider mb-4" style={{ color: colors.navy }}>
                Resources
              </h4>
              <ul className="space-y-3">
                <li>
                  <Link
                    href="/#funding"
                    className="footer-link font-body text-sm transition-colors hover:underline"
                    style={{ color: `${colors.navy}80` }}
                  >
                    News
                  </Link>
                </li>
                <li>
                  <Link
                    href="/privacy-policy"
                    className="footer-link font-body text-sm transition-colors hover:underline"
                    style={{ color: `${colors.navy}80` }}
                  >
                    Privacy Policy
                  </Link>
                </li>
                <li>
                  <Link
                    href="/safeguarding-policy"
                    className="footer-link font-body text-sm transition-colors hover:underline"
                    style={{ color: `${colors.navy}80` }}
                  >
                    Safeguarding Policy
                  </Link>
                </li>
              </ul>
            </div>

            {/* Contact */}
            <div>
              <h4 className="font-body text-xs font-bold uppercase tracking-wider mb-4" style={{ color: colors.navy }}>
                Contact
              </h4>
              <ul className="space-y-3">
                <li>
                  <a
                    href="mailto:hello@findaway.org"
                    className="footer-link font-body text-sm transition-colors hover:underline"
                    style={{ color: `${colors.navy}80` }}
                  >
                    hello@findaway.org
                  </a>
                </li>
                <li className="flex gap-3 pt-2">
                  <a
                    href="https://www.facebook.com/share/1HEMMP6LRs"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:scale-110"
                    style={{ backgroundColor: `${colors.navy}10`, color: colors.navy }}
                    aria-label="Facebook"
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                    </svg>
                  </a>
                  <a
                    href="https://www.instagram.com/findaway_psychology"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:scale-110"
                    style={{ backgroundColor: `${colors.navy}10`, color: colors.navy }}
                    aria-label="Instagram"
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                    </svg>
                  </a>
                  <a
                    href="https://x.com/findaway_psych"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:scale-110"
                    style={{ backgroundColor: `${colors.navy}10`, color: colors.navy }}
                    aria-label="X (Twitter)"
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                    </svg>
                  </a>
                </li>
              </ul>
            </div>
          </div>
        </div>

        <div
          className="pt-8 border-t flex flex-col md:flex-row justify-between items-center gap-4"
          style={{ borderColor: `${colors.navy}15` }}
        >
          <p className="font-body text-sm" style={{ color: `${colors.navy}60` }}>
            &copy; {new Date().getFullYear()} Find A Way: Psychological Services CIC. All rights reserved.
          </p>
          <p className="font-body text-sm" style={{ color: `${colors.navy}60` }}>
            Registered in England & Wales. Company no.{' '}
            <a
              href="https://find-and-update.company-information.service.gov.uk/company/15992869"
              className="underline hover:no-underline"
              target="_blank"
              rel="noopener noreferrer"
            >
              15992869
            </a>
          </p>
        </div>
      </div>
    </footer>
  );
}
